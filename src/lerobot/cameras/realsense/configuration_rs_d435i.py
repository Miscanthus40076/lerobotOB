...
def _validate_max_depth_m(max_depth_m: float) -> None:
    if max_depth_m <= 0:
        raise ValueError(f"`max_depth_m` is expected to be > 0, but {max_depth_m} is provided.")
...
class RealSenseD435iDepthCameraConfig(CameraConfig):
    ...
    def __post_init__(self) -> None:
        _validate_rotation(self.rotation)
        _validate_stream_shape(self.fps, self.width, self.height)
        _validate_max_depth_m(self.max_depth_m)
