...
from lerobot.cameras.realsense.configuration_rs_d435i import (
    RealSenseD435iColorCameraConfig,
    RealSenseD435iDepthCameraConfig,
)
from lerobot.cameras.utils import make_cameras_from_configs

d435i_module = importlib.import_module("lerobot.cameras.realsense.camera_rs_d435i")
...
class FakePipelineProfile:
    def __init__(self, resolved_streams: dict[str, FakeResolvedStream], requests: dict[str, dict[str, Any]]):
        self._resolved_streams = resolved_streams
        self._requests = requests

    def get_stream(self, stream_name: str) -> FakeVideoStreamProfile:
        frame_format = self._requests.get(stream_name, {}).get("format", "rgb8" if stream_name == "color" else "z16")
        return FakeVideoStreamProfile(self._resolved_streams[stream_name], frame_format)
...
class FakeConfig:
    ...
    def resolve(self, pipeline_wrapper: FakePipelineWrapper) -> FakePipelineProfile:
        ...
        return FakePipelineProfile(self._runtime.resolve_requests(self), self.requests)
...
class FakePipeline:
    ...
    def start(self, config: FakeConfig) -> FakePipelineProfile:
        ...
        self.started = True
        return FakePipelineProfile(resolved_streams, config.requests)
...
class FakeRealSenseModule:
    def __init__(self, runtime: "FakeRealSenseRuntime"):
        ...
        self.format = SimpleNamespace(rgb8="rgb8", bgr8="bgr8", yuyv="yuyv", z16="z16")
...
class FakeRealSenseRuntime:
    def __init__(self) -> None:
        ...
        self.supported_profiles = {
            "123456": {
                "color": [
                    FakeResolvedStream(width=2, height=3, fps=30),
        ...
        self.frames = {"123456": queue.Queue()}
...
def fake_realsense_runtime(monkeypatch: pytest.MonkeyPatch) -> FakeRealSenseRuntime:
    ...
    d435i_module.RealSenseD435iManager._registry.clear()
    monkeypatch.setattr(d435i_module, "rs", runtime.module)
    yield runtime
    d435i_module.RealSenseD435iManager._registry.clear()
    ...
    while time.time() < deadline:
        if camera.manager is not None:
            ...
            if frames is not None and seq > 0:
                return
        time.sleep(0.01)
    raise AssertionError("Timed out waiting for the fake RealSense manager to receive frames.")
    ...
    while time.time() < deadline:
        if camera.manager is not None:
            ...
            if seq >= min_seq:
                return
        time.sleep(0.01)
    raise AssertionError(f"Timed out waiting for the fake RealSense manager to reach seq>={min_seq}.")


def test_make_cameras_from_configs_supports_d435i_types(
    fake_realsense_runtime: FakeRealSenseRuntime,
) -> None:
    cameras = make_cameras_from_configs(
        {
            "color": RealSenseD435iColorCameraConfig(
                serial_number_or_name="123456", width=640, height=480, fps=30
            ),
            "depth": RealSenseD435iDepthCameraConfig(
                serial_number_or_name="123456", width=640, height=480, fps=30
            ),
        }
    )

    assert isinstance(cameras["color"], d435i_module.RealSenseD435iColorCamera)
    assert isinstance(cameras["depth"], d435i_module.RealSenseD435iDepthCamera)


def test_connect_reuses_manager_and_restarts_when_depth_is_added(
    fake_realsense_runtime: FakeRealSenseRuntime,
) -> None:
    color = d435i_module.RealSenseD435iColorCamera(
        RealSenseD435iColorCameraConfig(serial_number_or_name="123456", width=640, height=480, fps=30)
    )
    depth = d435i_module.RealSenseD435iDepthCamera(
        RealSenseD435iDepthCameraConfig(serial_number_or_name="123456", width=640, height=480, fps=30)
    )
    ...
    depth.disconnect()
    assert fake_realsense_runtime.stop_calls == 2
    assert not d435i_module.RealSenseD435iManager._registry
...
def test_duplicate_connect_does_not_double_open_device(
    fake_realsense_runtime: FakeRealSenseRuntime,
) -> None:
    color = d435i_module.RealSenseD435iColorCamera(
        RealSenseD435iColorCameraConfig(serial_number_or_name="123456", width=640, height=480, fps=30)
...
def test_connect_rejects_mismatched_shared_fps(fake_realsense_runtime: FakeRealSenseRuntime) -> None:
    ...
    depth = d435i_module.RealSenseD435iDepthCamera(
        RealSenseD435iDepthCameraConfig(serial_number_or_name="123456", width=640, height=480, fps=60)
    )
...
def test_connect_rejects_unsupported_device(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    runtime.frames = {"777": queue.Queue()}
    d435i_module.RealSenseD435iManager._registry.clear()
    monkeypatch.setattr(d435i_module, "rs", runtime.module)

    color = d435i_module.RealSenseD435iColorCamera(
        RealSenseD435iColorCameraConfig(serial_number_or_name="777", width=640, height=480, fps=30)
    )

    with pytest.raises(ValueError, match="only supports D435i"):
        color.connect(warmup=False)
...
def test_color_and_depth_processing_outputs_expected_images(
    fake_realsense_runtime: FakeRealSenseRuntime,
) -> None:
    color = d435i_module.RealSenseD435iColorCamera(
        RealSenseD435iColorCameraConfig(
    ...
    fake_realsense_runtime.push_frame("123456", color=color_raw, depth=depth_raw)
    ...
    expected_color = cv2.rotate(cv2.cvtColor(color_raw, cv2.COLOR_RGB2BGR), cv2.ROTATE_90_CLOCKWISE)
    assert np.array_equal(color_frame, expected_color)

    expected_depth_8u = np.zeros(depth_raw.shape, dtype=np.uint8)
    valid_mask = (depth_raw > 0) & (depth_raw <= int(round(depth.config.max_depth_m * 1000.0)))
    valid_depth = depth_raw[valid_mask].astype(np.float32)
    depth_min = float(valid_depth.min())
    depth_max = float(valid_depth.max())
    expected_depth_8u[valid_mask] = ((valid_depth - depth_min) * 255.0 / (depth_max - depth_min)).astype(np.uint8)
    expected_depth = cv2.applyColorMap(expected_depth_8u, cv2.COLORMAP_JET)
    expected_depth[depth_raw == 0] = 0
    expected_depth = cv2.rotate(expected_depth, cv2.ROTATE_90_CLOCKWISE)
...
def test_depth_processing_clamps_values_beyond_max_depth_m(
    fake_realsense_runtime: FakeRealSenseRuntime,
) -> None:
    depth = d435i_module.RealSenseD435iDepthCamera(
        RealSenseD435iDepthCameraConfig(
    ...
    fake_realsense_runtime.push_frame("123456", depth=depth_raw)
    ...
    expected_depth_8u = np.zeros(depth_raw.shape, dtype=np.uint8)
    valid_mask = (depth_raw > 0) & (depth_raw <= 2000)
    valid_depth = depth_raw[valid_mask].astype(np.float32)
    depth_min = float(valid_depth.min())
    depth_max = float(valid_depth.max())
    expected_depth_8u[valid_mask] = ((valid_depth - depth_min) * 255.0 / (depth_max - depth_min)).astype(np.uint8)
    expected_depth = cv2.applyColorMap(expected_depth_8u, cv2.COLORMAP_JET)
    expected_depth[~valid_mask] = 0
    assert np.array_equal(depth_frame, expected_depth)

    depth.disconnect()


def test_async_read_starts_background_thread_and_returns_latest_frame(
    fake_realsense_runtime: FakeRealSenseRuntime,
) -> None:
    color = d435i_module.RealSenseD435iColorCamera(
        RealSenseD435iColorCameraConfig(
    ...
    fake_realsense_runtime.push_frame("123456", color=np.full((3, 2, 3), 20, dtype=np.uint8))
    frame = color.async_read(timeout_ms=500)

    assert frame.shape == (3, 2, 3)
    assert color.thread is not None
    assert color.thread.is_alive()
    assert color.manager is not None
    assert color.manager.get_status()["thread_alive"] is True
